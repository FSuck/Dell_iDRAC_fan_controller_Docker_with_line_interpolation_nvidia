...
# Log the check interval, fan speed objective and CPU/GPU temperature threshold
...
if $FAN_SPEED_INTERPOLATION_ENABLED; then
  ...
  echo "GPU lower temperature threshold: \"$GPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION\"°C"
  echo "GPU higher temperature threshold: \"$GPU_TEMPERATURE_THRESHOLD\"°C"
  echo ""
...
IS_GPU_TEMPERATURE_SENSOR_PRESENT=true
retrieve_temperatures $IS_EXHAUST_TEMPERATURE_SENSOR_PRESENT $IS_CPU2_TEMPERATURE_SENSOR_PRESENT $IS_GPU_TEMPERATURE_SENSOR_PRESENT
...
if ! [[ "$GPU_TEMPERATURE" =~ ^[0-9]+$ ]]; then
  echo "No Nvidia GPU temperature sensor detected."
  IS_GPU_TEMPERATURE_SENSOR_PRESENT=false
fi
# Output new line to beautify output if one of the previous conditions have echoed
if ! $IS_EXHAUST_TEMPERATURE_SENSOR_PRESENT || ! $IS_CPU2_TEMPERATURE_SENSOR_PRESENT || ! $IS_GPU_TEMPERATURE_SENSOR_PRESENT; then
  echo ""
fi
...
while true; do
  ...
  # If CPU 2 temperature sensor is present, check if it is overheating then apply Dell default dynamic fan control profile if true
  ...
  elif CPU1_HEATING || $IS_CPU2_TEMPERATURE_SENSOR_PRESENT && CPU2_HEATING || $IS_GPU_TEMPERATURE_SENSOR_PRESENT && GPU_HEATING; then

    # Get fan speed according to cpu temperature
    ...
    DECIMAL_FAN_SPEED_TO_APPLY_FOR_CPU=$(calculate_interpolated_fan_speed "$DECIMAL_LOW_FAN_SPEED_OBJECTIVE" "$DECIMAL_HIGH_FAN_SPEED_OBJECTIVE" "$HIGHEST_CPU_TEMPERATURE" "$CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION" "$CPU_TEMPERATURE_THRESHOLD")

    # Get fan speed according to gpu temperature
    DECIMAL_FAN_SPEED_TO_APPLY_FOR_GPU=$(calculate_interpolated_fan_speed "$DECIMAL_LOW_FAN_SPEED_OBJECTIVE" "$DECIMAL_HIGH_FAN_SPEED_OBJECTIVE" "$GPU_TEMPERATURE" "$GPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION" "$GPU_TEMPERATURE_THRESHOLD")

    if (( DECIMAL_FAN_SPEED_TO_APPLY_FOR_CPU > DECIMAL_FAN_SPEED_TO_APPLY_FOR_GPU )); then
      DECIMAL_FAN_SPEED_TO_APPLY=$DECIMAL_FAN_SPEED_TO_APPLY_FOR_CPU
    else
      DECIMAL_FAN_SPEED_TO_APPLY=$DECIMAL_FAN_SPEED_TO_APPLY_FOR_GPU
    fi
